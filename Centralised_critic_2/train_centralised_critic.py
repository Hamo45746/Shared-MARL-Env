...
from centralised_critic_model import CentralisedCriticModel  # Assuming your custom model is in this file
...
# Register your environment (if not already done)
...
# Define the policy mapping function - centralised training
...
obs_space = spaces.Dict({
    ...
    "other_agents_obs": spaces.Box(low=-np.inf, high=np.inf, shape=(num_agents-1, 4, 32), dtype=np.float32)
})
# This is the config for cnetralised training - one policy
config["multiagent"]["policies"] = {
    "policy_0": (None, obs_space, action_space, {})
}
# Update the configuration for the centralized critic model
config["multiagent"]["policy_mapping_fn"] = policy_mapping_fn

# Update the model config
config["model"] = {
    "custom_model": "centralised_critic_model",
    "custom_model_config": {
        "num_agents": num_agents,
        "obs_dim": np.prod(obs_shape),  # For example, 4*32=128
...
for i in range(100):
    ...
    if (i + 2) % 25 == 0:
        ...
        env2.run_simulation_with_centralised_critic(checkpoint_dir=checkpoint_dir, params_path=params_path, max_steps=900, iteration=i)

# Save the final checkpoint
final_checkpoint_dir = trainer.save()
