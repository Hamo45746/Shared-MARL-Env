...
class AgentLayer:
    ...
    def move_agent(self, agent_idx, action):
        ...
        self.agent_positions[agent_idx] = n_pos
        return n_pos
    ...
    def remove_agent(self, agent_idx):
        ...
        if 0 <= agent_idx < self.nagents:
            ...
            self.nagents -= 1
            self.layer_state[pos[0], pos[1]] = -20.0
            self.agent_positions = np.delete(self.agent_positions, agent_idx, axis=0)
    ...
    def update(self):
        mask = (self.layer_state <= 0.0) & (self.layer_state > -20.0)
        self.layer_state[mask] -= 0.1
        self.layer_state[self.layer_state < -20.0] = -20.0
        self.update_agent_positions()
        self.layer_state[self.agent_positions[:, 0], self.agent_positions[:, 1]] = 0.0
...
class TargetLayer:
    ...
    def update_position(self, old_position, new_position):
        ox, oy = tuple(map(int, old_position))
        nx, ny = tuple(map(int, new_position))
        if self.layer_state[ox, oy] == 0.0:  # Only reset if it was the current position of the target
            self.layer_state[ox, oy] = -0.1  # Start decay from -0.1
        self.layer_state[nx, ny] = 0.0  # Set new position to 0 (target present)
    ...
    def update(self):
        ...
        for target in self.targets:
            ...
            self.layer_state[x, y] = 0.0  # Set current agent positions to 0
...
class JammerLayer:
    ...
    def update_layer_state(self):
        ...
        """
        self.layer_state.fill(-20.0)
        for jammer in self.jammers:
            x, y = jammer.current_position()
            if jammer.is_active():
                self.layer_state[x, y] = 0.0
