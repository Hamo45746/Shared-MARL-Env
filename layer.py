...
class AgentLayer:
    ...
    def update_position(self, old_position, new_position):
        ...
        if self.layer_state[ox, oy] == 0.0:  # Only reset if it was the current position of the agent
            self.layer_state[ox, oy] = -0.1  # Start decay from -0.1
        self.layer_state[nx, ny] = 0.0  # Refresh the new position to 0

    def set_position(self, agent_idx, x, y):
        o_pos = self.agents[agent_idx].current_position()
        self.agents[agent_idx].set_position(x, y)
        n_pos = np.zeros(2, dtype=np.float16)
        n_pos[0], n_pos[1] = x, y
        self.update_position(o_pos, n_pos)
    ...
    def remove_agent(self, agent_idx):
        ...
        if 0 <= agent_idx < self.nagents:
            ...
            self.nagents -= 1
            self.layer_state[int(pos[0]), int(pos[1])] = -20.0 # Clear the position in the layer state
